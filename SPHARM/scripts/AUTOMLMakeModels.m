function [currentDir] = AUTOMLMakeModels(currentDir, x, y, z, inputFile, smoothFile, translation)
    % Make SPHARM models.
    ...
    if (toFile)
        outputTextFileName = strcat('output_parameters_degree_', num2str(degree), '.txt');
        textDir = currentDir;
    ...
    for i = 1:n;
        ...
        if (toFile)
            ...
            for k = 1:rows
                ...
                if (imag(fvec(k, 1)) < 0)
                    fprintf(fid, '\t%.16f %.16fi', real(fvec(k, 1)), imag(fvec(k, 1)));
                else
                    fprintf(fid, '\t%.16f +%.16fi', real(fvec(k, 1)), imag(fvec(k, 1)));
                end

                if (imag(fvec(k, 2)) < 0)
                    fprintf(fid, '\t%.16f %.16fi', real(fvec(k, 2)), imag(fvec(k, 2)));
                else
                    fprintf(fid, '\t%.16f +%.16fi', real(fvec(k, 2)), imag(fvec(k, 2)));
                end

                if (imag(fvec(k, 3)) < 0)
                    fprintf(fid, '\t%.16f %.16fi', real(fvec(k, 3)), imag(fvec(k, 3)));
                else
                    fprintf(fid, '\t%.16f +%.16fi', real(fvec(k, 3)), imag(fvec(k, 3)));
                end
