function disp_surfmesh(name,vertices,faces,varargin)
...
if ~isempty(varargin)
    poles = varargin{1};
    plot3(vertices(poles,1),vertices(poles,2),vertices(poles,3),'o');
    text(vertices(poles(1),1),vertices(poles(1),2),vertices(poles(1),3),'n','FontSize',18);
    text(vertices(poles(2),1),vertices(poles(2),2),vertices(poles(2),3),'s','FontSize',18);
end;    
axis off;
hold off;
