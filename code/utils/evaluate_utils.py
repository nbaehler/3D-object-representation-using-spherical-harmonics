...
def cuda_get_rasterize():
    dll = ctypes.CDLL(
        '/cvlabdata2/home/wickrama/projects/U-Net/Experiments/meshnet/mnet/kernel.so', mode=ctypes.RTLD_GLOBAL)
...
def prepare_run(vertices, faces, grid_size):
    ...
    (D, H, W) = grid_size
    v = torch.tensor([D-1, H-1, W-1])[None].float() * \
        (torch.flip(vertices.clone(), [1]) + 1)/2
