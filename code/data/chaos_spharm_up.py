...
class Chaos():
    ...
    def sample_to_sample_plus(self, samples, cfg, datamode):
        ...
        for sample in samples: 
            ...
            new_samples += [SamplePlus(x.cpu(), y.cpu(), sample.name, sample.spharm_coeffs)]
    ...
    def prepare_data(self, cfg):
        ...
        for i, datamode in enumerate([DataModes.TRAINING, DataModes.TESTING]):
            ...
            for j in counts[i]:
                ...
                sample.x = F.interpolate(sample.x[None, None], scale_factor=scale_factor, mode='trilinear')[0, 0]
                sample.y = F.interpolate(sample.y[None, None].float(), scale_factor=scale_factor, mode='nearest')[0, 0].long()

                new_samples.append(sample)

            with open(cfg.data_path + 'prepared_data_{}_{}.pickle'.format(datamode, "_".join(map(str, down_sample_shape))), 'wb') as handle:
    ...
    def create_obj_files(self, cfg, samples, datamode):
        ...
        for sample in samples:
            ...
            vertices, faces = voxel2mesh(y_, sample.step_size, torch.tensor(sample.y.shape)[None].float())

            # Save to obj file for meshlab
            # Points = V x 3 
            # Points = 1 x V x 3 
            # Faces =  1 x F x 3 
            save_to_obj(save_path + 'label_' + sample.name + '.obj', vertices[None], faces[None])
                ...
                with open(inputFileName, 'r') as input:
                    ...
                    while line:
                        ...
                        if (elems[0] == 'v'):
                            ...
                            outputContent = addLine(outputContent, elems[1], elems[2], elems[3])
    ...
    def import_params(self, cfg):
        ...
        for i, datamode in enumerate([DataModes.TRAINING, DataModes.TESTING]):
            ...
            for j in range(len(prepare_samples)):
                ...
                params_path = cfg.data_path+'reconstructions/'+datamode+'/label_'+name+'/output_parameters_degree_'+str(cfg.spharm_degree)+'.txt'

                spharm_coeffs = load_spharm_coeffs_from_file(params_path)

                samples.append(Sample(x, y, name, spharm_coeffs))

            with open(cfg.data_path + 'extended_data.pickle', 'wb') as handle:
    ...
    def evaluate(self, target, pred, cfg):
        ...
        if target.points is not None:
            results['chamfer_weighted_symmetric'] = chamfer_distance(target.points, pred.points)[0].item()
