...
def write_to_wandb(writer, epoch, split, performances, num_classes): 
    ...
    for key, value in performances[split].items():
        ...
        for i in range(1, num_classes):
            log_vals[split + '_' + key + '/class_' + str(i)] = np.mean(performances[split][key][:, i - 1]) 
    try:
        wandb.log(log_vals)
    except:
        print('')
...
class Evaluator(object):
    ...
    def predict(self, data, config):
        ...
        if name == 'unet':
            ...
            true_meshes += [{'vertices': data['vertices_mc'], 'faces': data['faces_mc'], 'normals':None}]
            true_points += [data['surface_points']]
            true_voxels += [data['y_voxels']]

            x = x.detach().data.cpu()
            y = Structure(mesh=true_meshes, voxel=true_voxels, points=true_points) #, voxel_rasterized=true_voxels)# TODO true mesh (dict) marching cube, voxel binary mask, points 1xNx3
    ...
    def evaluate_set(self, dataloader):
        ...
        for i, data in enumerate(dataloader):
            ...
            dataSamples.append(DataSample(x=x, y=y, name=data['name'][0], y_hat_spharm_coeffs=y_hat_spharm_coeffs))
    ...
    def save_incomplete_evaluations(self):
        ...
        for eval in self.evaluations:
            save_path = self.config.data_path+'evaluations/'+str(eval.iteration)+'/'
    ...
    def do_complete_evaluations(self, data_obj, cfg):
        ...
        for eval in evaluations:
            ...
            for i, data in enumerate(eval.data):
                ...
                data.y.mesh = data.y.mesh[0] # For now only one in the list TODO
                ...
                pred_meshes = {'vertices': vertices, 'faces': faces, 'normals': None}

                vertices = torch.Tensor([vertices])
                faces = torch.Tensor([faces])

                mesh = Meshes(vertices, faces)
                pred_points = sample_points_from_meshes(mesh, cfg.samples_for_chamfer)
                ...
                y_hat = Structure(mesh=pred_meshes, voxel=pred_voxels, points=pred_points)#, voxel_rasterized=pred_voxels) TODO
                ...
                mesh = form_mesh(np.array(pred_meshes['vertices']), np.array(pred_meshes['faces']))
            ...
            chamfer.write(str(eval.iteration)+' '+str(dist/len(eval.data))+'\n')
    ...
    def save_results(self, predictions, epoch, performance, save_path, mode):
        ...
        if performance is not None:
            for key, value in performance.items():
                ...
                summary = ('{}: ' + ', '.join(['{:.8f}' for _ in range(self.config.num_classes-1)])).format(epoch, *performance_mean)
                append_line(save_path + mode + 'summary' + key + '.txt', summary)
                print(summary)

                all_results = [('{}: ' + ', '.join(['{:.8f}' for _ in range(self.config.num_classes-1)])).format(*((i+1,) + tuple(vals))) for i, vals in enumerate(performance[key])]
