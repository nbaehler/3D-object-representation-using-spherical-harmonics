...
class SPHarmNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        max_pool = nn.MaxPool3d(4) if config.ndims == 3 else nn.MaxPool2d(4)
        ...
        feature_count = np.prod(
            out_shape) * config.first_layer_channels * 2**config.steps

        fc_layers = [nn.Linear(feature_count, feature_count//4)]
        fc_layers.append(nn.Linear(feature_count//4, feature_count//16))
        fc_layers.append(nn.Linear(feature_count//16,
