...
class SPHarmNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        '''  Down layers '''
        down_layers = [Layer(config.num_input_channels, config.first_layer_channels, config.ndims)]
        for i in range(1, config.steps + 1):
            down_layers.append(max_pool)
            lyr = Layer(config.first_layer_channels * 2**(i - 1), config.first_layer_channels * 2**i, config.ndims, config.batch_norm)
        ...
        fc_layers = []
        fc_layers.append(nn.Linear(feature_count, feature_count//8))
