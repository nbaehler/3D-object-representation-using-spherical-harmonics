...
class UNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i in range(config.steps - 1, -1, -1):
            upconv = ConvTransposeLayer(in_channels=config.first_layer_channels * 2**(
                i+1), out_channels=config.first_layer_channels * 2**i, kernel_size=2, stride=2)
            lyr = UNetLayer(config.first_layer_channels * 2**(i + 1),
                            config.first_layer_channels * 2**i, config.ndims, config.batch_norm)
            up_layers.append((upconv, lyr))

        ''' Final layer '''
        final_layer = ConvLayer(in_channels=config.first_layer_channels,
                                out_channels=config.num_classes, kernel_size=1)
