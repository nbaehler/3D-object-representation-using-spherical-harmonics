...
typedef struct smoparam_struct
{
	...
	int ow;
	int nw; /* (ow*old + nw*new)/(ow+nw) => new   */
} SMOPARAM;
...
void locsmo(MAPPING *map, int iter)
{
	...
	{ /* do iterations */
		...
		{ /* traverse the surface */
			if (relocate(newpvs, map->param_vs, i, map))
				changed = TRUE;
		}
		if (changed)
		{
...
double *extract_locmesh_verts(double *verts, int vnum, int ix, int *nbvs, int nbnum)
{
	...
	lvsx++;
	lvsy++;
	lvsz++; /* skip the first element, which is the center  */
	for (i = 0; i < nbnum; i++)
	{
		lvsx[i] = vsx[nbvs[i]];
		lvsy[i] = vsy[nbvs[i]];
...
void create_obj_area(MAPPING *map)
{
	...
	for (i = 0; i < fnum; i++)
		area[i] = area[i] / ttarea;

	for (i = 0; i < vnum; i++)
		vtarea[i] = vtarea[i] / ttarea;
...
void solve_dgels(LEQS *leqs)
{
	...
	{
		BLASCALL(dgels)
		("N", &m, &n, &NRHS, A, &m, b, &m, WORK, &LWORK, &ok);
	}
	else
	{
		printf(">>> New lwork=%d\n", lwork);
