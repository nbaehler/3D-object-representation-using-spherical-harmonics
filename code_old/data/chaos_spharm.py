...
class Chaos:
    ...
    def load_data(self, cfg):
        ...
        for sample in samples:
            if "pickle" not in sample:
                ...
                x = F.grid_sample(
                    x[None, None], grid, mode="bilinear", padding_mode="border"
                )[0, 0]
                x = x.data.cpu().numpy()
                # ----
                ...
                y = F.grid_sample(
                    y[None, None].float(), grid, mode="nearest", padding_mode="border"
                )[0, 0]
                y = y.data.cpu().numpy()
    ...
    def prepare_data(self, cfg):
        ...
        for i, datamode in enumerate([DataModes.TRAINING, DataModes.TESTING]):
            ...
            with open(
                cfg.data_path + "prepared_data_" + datamode + ".pickle", "wb"
            ) as handle:
    ...
    def import_params(self, cfg):
        ...
        for i, datamode in enumerate([DataModes.TRAINING, DataModes.TESTING]):
            ...
            with open(
                cfg.data_path + "prepared_data_" + datamode + ".pickle", "rb"
            ) as handle:
